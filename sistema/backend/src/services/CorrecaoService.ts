import { ResultadoCorrecaoDTO } from "../models/Prova";
...
export class CorrecaoService {
  ...
  async corrigir(
    ...
    for (const questaoProva of prova.questoes) {
      ...
      const respostaUsuario = respostasDoUsuario.find(
      ...
      if (!respostaUsuario) {
        detalhes.push({
          ...
          respostaCorreta: this.gerarRespostaCorreta(
            questao.alternativas,
            prova.tipoDeResposta
          ),
        });
        continue;
      }

      const respostaCorreta = this.gerarRespostaCorreta(
        questao.alternativas,
        prova.tipoDeResposta
      );
      const estaCorreta =
        this.normalizarResposta(respostaUsuario.resposta) ===
    ...
    const percentualAcerto =
      (questoesCorretas / prova.questoes.length) * 100;
  ...
  private normalizarResposta(resposta: string): string {
    return resposta
      .toLowerCase()
      .trim()
      .replace(/\s+/g, "")
