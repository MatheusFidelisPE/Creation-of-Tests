...
export class CorrecaoCSVService {
  // Parsear CSV e converter em objeto chave-valor
  private parseCSV(conteudo: string): Record<string, string[]> {
  ...
  private extrairAlternativasCorretas(valor: string, tipo: 'LETRAS' | 'SOMA_EXPONENCIAL'): Set<number> {
    const alternativas = new Set<number>();
  ...
  private calcularPontuacaoQuestao(
    resposta: string,
    gabarito: string,
    tipo: 'LETRAS' | 'SOMA_EXPONENCIAL',
    rigor: boolean
  ): number {
  ...
  async corrigirProvas(
    gabaritosConteudo: string,
    respostasConteudo: string,
    ...
    for (const [idProva, respostasQuestoes] of Object.entries(respostas)) {
      ...
      if (respostasQuestoes.length !== gabaritosQuestoes.length) {
        console.warn(
          `Número de questões diferente para prova ${idProva}: ` +
